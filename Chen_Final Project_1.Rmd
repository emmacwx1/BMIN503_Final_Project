...
```
## Notes: comparing PCA of UMAP and tSNE clusters, cluster's immune profiles and distribution is quite different. (cluster number of cells sharing similar immune profil might be different because they are from two different algorithm)


I am also interested to see specific protein relationship across different time points: 
C2: only anti-PD1
C3: both JAK inhibitor and anti-PD1
...
#see if CD4 and CD8 T cells are well-separated
ggplot(data = c2, aes(x = CD4, y = CD8 )) +
    geom_point(color = "pink")

#exhaustion marker TOX vs wnt-signaling TCF1
ggplot(data = c2, aes(x = Tox, y = TCF1 )) +
    geom_point(color = "blue")
#does not seem to have a separate TCF+ population-->but it could be due to mapping not using log


#IgG4 is used to mark PD1+ cells as the drug already "blocked" PD-1 receptor
ggplot(data = c2, aes(x = PD1, y = IgG4 )) +
    geom_point(color = "green")


#TIGIT inhibitory, CD226 activation
...
repeat this data cleaning and analysis for c3, c4 and c5
## c3: first cycle with both JAK and anti-PD1
```{r}
c3 <-  dir(pattern="export_export3_Exhastion_3A_030.fcs")

c3 = read.FCS(c3, transformation="linearize", which.lines=NULL,
         alter.names=FALSE, column.pattern=NULL, invert.pattern = FALSE,
         decades=0, ncdf = FALSE, min.limit=NULL, 
         truncate_max_range = TRUE, dataset=NULL, emptyValue=TRUE, 
         channel_alias = NULL)
...
c3 = exprs(c3)
c3 = c3[, c(7:31)]
colnames(c3) <- c("CD45RA", "CXCR5", "PD1", "IgG4", "dump", "CD8", "CD127", "Tox", "Tim3", "Eomes", "TCF1", "CD38", "CCR7", "CTLA-4", "CD226", "CD39", "FoxP3", "Ki67", "CD27", "CD4", "CD73", "TIGIT", "T-bet", "CD28", "CD3")
```
...
#see if CD4 and CD8 T cells are well-separated
ggplot(data = c3, aes(x = CD4, y = CD8 )) +
    geom_point(color = "pink")

#exhaustion marker TOX vs wnt-signaling TCF1
ggplot(data = c3, aes(x = Tox, y = TCF1 )) +
    geom_point(color = "blue")
#does not seem to have a separate TCF+ population-->but it could be due to mapping not using log


#IgG4 is used to mark PD1+ cells as the drug already "blocked" PD-1 receptor
ggplot(data = c3, aes(x = PD1, y = IgG4 )) +
    geom_point(color = "green")


#TIGIT inhibitory, CD226 activation
ggplot(data = c3, aes(x = TIGIT, y = CD226 )) +
    geom_point(color = "yellow")
#does not seem to have a separate 
...
## c4: last/second cycle with both JAK and anti-PD1
```{r}
...
## c5: first second getting back to only anti-PD1
```{r}
c5 <-  dir(pattern="export_export3_Exhastion_4A_031.fcs")

c5 = read.FCS(c5, transformation="linearize", which.lines=NULL,
         alter.names=FALSE, column.pattern=NULL, invert.pattern = FALSE,
         decades=0, ncdf = FALSE, min.limit=NULL, 
         truncate_max_range = TRUE, dataset=NULL, emptyValue=TRUE, 
         channel_alias = NULL)
...
c5 = exprs(c5)
c5 = c5[, c(7:31)]
colnames(c5) <- c("CD45RA", "CXCR5", "PD1", "IgG4", "dump", "CD8", "CD127", "Tox", "Tim3", "Eomes", "TCF1", "CD38", "CCR7", "CTLA-4", "CD226", "CD39", "FoxP3", "Ki67", "CD27", "CD4", "CD73", "TIGIT", "T-bet", "CD28", "CD3")
```
...
#see if CD4 and CD8 T cells are well-separated
ggplot(data = c5, aes(x = CD4, y = CD8 )) +
    geom_point(color = "pink")

#exhaustion marker TOX vs wnt-signaling TCF1
ggplot(data = c5, aes(x = Tox, y = TCF1 )) +
    geom_point(color = "blue")
#does not seem to have a separate TCF+ population-->but it could be due to mapping not using log


#IgG4 is used to mark PD1+ cells as the drug already "blocked" PD-1 receptor
ggplot(data = c5, aes(x = PD1, y = IgG4 )) +
    geom_point(color = "green")


#TIGIT inhibitory, CD226 activation
ggplot(data = c5, aes(x = TIGIT, y = CD226 )) +
    geom_point(color = "yellow")
#does not seem to have a separate 
...
Immune profiling: 
just from these two PCA plots, it seems UMAP and tSNE have very different clustering results, need to look into marker expressions of clusters.
