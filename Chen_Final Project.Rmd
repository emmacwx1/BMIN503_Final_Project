...
#concatenate_fcs_files(concat, output.file = "C:/Users/Emma/Documents/BMIN503_Final_Project/concatnated.csv")
...
tSNE.data = as.data.frame(mytSNE$Y) #Matrix containing the new representations for the objects
...
#fileName <- system.file("concat", package="FlowSOM")
fSOM <- ReadInput(concat, compensate = FALSE, transform = TRUE,
                    toTransform=c(7:31), scale = FALSE)
ff <- suppressWarnings(flowCore::read.FCS(concat))
fSOM <- ReadInput(ff, compensate = TRUE, transform = TRUE, scale = TRUE)

fSOM <- BuildSOM(fSOM, colsToUse = c(12, 26))
fSOM <- BuildMST(fSOM, tSNE=TRUE)
cluster_labels <- GetClusters(fSOM)
mfis <- GetMFIs(fSOM, colsUsed = 7:31, prettyColnames = TRUE)


PlotStars(fSOM$FlowSOM, markers = fsom$map$colsUsed, view = "MST",
colorPalette = grDevices::colorRampPalette(c("#00007F", "blue",
"#007FFF", "cyan", "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000")),
starBg = "white", backgroundValues = NULL,
backgroundColor = function(n) { grDevices::rainbow(n, alpha = 0.3)
}, backgroundLim = NULL, backgroundBreaks = NULL,
backgroundSize = NULL, thresholds = NULL, legend = TRUE,
query = NULL, main = "")
...
write.csv(flowSOM.clustering, "C:/Users/Emma/Documents/BMIN503_Final_Project/clusters_concat.csv")
#first column is cell event, second column in cluster ID
...
```{r MEM on UMAP/FlowSOM Clusters  eval = FALSE}
# Run MEM on the FlowSOM clusters from UMAP
...
```{r RMSD for All Clusters eval = FALSE}
