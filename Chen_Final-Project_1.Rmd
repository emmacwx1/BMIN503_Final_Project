...
RMSD calculate pair-wise cluster relationship. It determines which markers the population share in common and then calculates the percent maximum RMSD between the two populations

for next direction, can used to compare clusters from different timepoints of the same patients, and between patients. the more red means the clusters have more enrichment MEM scores in common. 
...
2-and it takes a longer time to compute
...
Completed tasks are: 
Alternative and potentially more efficient way to analyze cytometry data (Flow & CyTOF)
Create workflow for analyzing longitudinal study data 
Evaluated dimension reduction tool/packages
